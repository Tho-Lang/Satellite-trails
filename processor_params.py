import sys
sys.path.insert(0,"/lfs/l7/hess/users/spencers/realdata/root_build/lib")
import ROOT
import os
import tables
import numpy as np
...
outfilename = outdir+'/'+str(runnumber)+'_params.hdf5'
...
while dataset_iterator != events.end():
    ...
    while tels_iterator != tels_with_data.end():
        ...
        hillas = hess.Get(tels_iterator.GetId(),cleaning, ROOT.Reco.HillasParameters())
    ...
    if len(ev_twithdata)>0:
        ...
        hillascogs.append(ev_hillascog)
        hillasphis.append(ev_hillasphi)
        hillaslds.append(ev_hillasld)
        time_second.append(event_header.GetTimeStamp().fTime)
...
writefile(h5file,'time_second',time_second)
writefile(h5file,'time_nanosecond',time_nanosecond)
writefile(h5file,'cleaning',['cleaning'])
